...
/**
 * 有输入提示的输入框 本质就是Select组件
 *
 * 1. 自定义输入组件，设置Select.mode = SECRET_COMBOBOX_MODE_DO_NOT_USE, 并且设置getInputElement
 * 2. 使用Option包裹options选项
 */
const AutoComplete: React.ForwardRefRenderFunction<RefSelectProps, AutoCompleteProps> = (
  props,
  ref,
) => {
  const { prefixCls: customizePrefixCls, className, children, dataSource } = props; // version4之后，dataSource改为options
  ...
  // 对children和options封装，保证其被Option组件包裹
