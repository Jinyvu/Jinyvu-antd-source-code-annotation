...
import defaultRenderEmpty from '../config-provider/defaultRenderEmpty'; // 默认的空状态
...
/**
 * DefaultOptionType label: React.ReactNode; value?: string | number | null; children?:
 * DefaultOptionType[];
 */
...
/** 输入一串字符串，做类似如下处理 input: ("a/b/c", "/") output: ["a", <span>/</span>, "b", <span>/</span>, "c"] */
...
export type CascaderProps<DataNodeType> = UnionCascaderProps & {
  multiple?: boolean; // 是否可以复选
  size?: SizeType; // 输入框大小
  disabled?: boolean;
  bordered?: boolean; // 是否有边框
  placement?: SelectCommonPlacement; // 浮层预设位置
  suffixIcon?: React.ReactNode; // 自定义的选择框后缀图标
  options?: DataNodeType[]; // 可选项数据源
  status?: InputStatus;
};
...
/** 初始化rc-cascader需要的参数，再返回使用这些参数的rc-cascader */
