...
> extends Omit<
    ...
    | 'switcherIcon'
  > {
  suffixIcon?: React.ReactNode; // 自定义的选择框后缀图标, 多选模式下必须同时设置 showArrow 为 true
  size?: SizeType;
  disabled?: boolean;
  placement?: SelectCommonPlacement; // 选择框弹出的位置
  bordered?: boolean; // 是否显示边框
  treeLine?: TreeProps['showLine']; // 是否展示线条样式
  status?: InputStatus;
  switcherIcon?: SwitcherIcon | RcTreeSelectProps<ValueType, OptionType>['switcherIcon']; // 自定义树节点的展开/折叠图标
}
...
const InternalTreeSelect = <OptionType extends BaseOptionType | DefaultOptionType = BaseOptionType>(
  {
    ...
    switcherIcon,
    treeLine,
    getPopupContainer, // 菜单渲染父节点，默认body
    dropdownClassName,
    treeIcon = false,
    transitionName,
    choiceTransitionName = '',
    status: customStatus,
    showArrow, // 	是否显示 suffixIcon，单选模式下默认 true
    treeExpandAction, // 点击节点 title 时的展开逻辑，可选：false | click | doubleClick
    ...props
